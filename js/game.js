...
// --- Game Classes ---
class Player {
    constructor(id, controls, sprite) { this.id = id; this.width = 40; this.height = 40; this.x = GAME_WIDTH / 2 - this.width / 2 + (id === 1 ? -50 : 50); this.y = GAME_HEIGHT - this.height - 50; this.vx = 0; this.vy = 0; this.onGround = false; this.sprite = sprite; this.onFrozenPlatform = false; this.controls = controls; this.isDead = false; this.score = 0; this.hp = 1000; this.maxHp = 1000; }
    resetPosition() { this.x = GAME_WIDTH / 2 - this.width / 2 + (this.id === 1 ? -50 : 50); this.y = GAME_HEIGHT - this.height - 50; this.vx = 0; this.vy = 0; if (this.hp <= 0) this.hp = this.maxHp; }
    ...
    update() { if (this.isDead) return; if (keys[this.controls.jump] && this.onGround) { this.jump(); } if (this.onFrozenPlatform) { if (!keys[this.controls.left] && !keys[this.controls.right]) { this.vx *= 0.97; if (Math.abs(this.vx) < 0.1) this.vx = 0; } else { if (keys[this.controls.left]) this.vx = -PLAYER_SPEED; if (keys[this.controls.right]) this.vx = PLAYER_SPEED; } } else { this.vx = 0; if (keys[this.controls.left]) this.vx = -PLAYER_SPEED; if (keys[this.controls.right]) this.vx = PLAYER_SPEED; } this.x += this.vx; if (this.x < -this.width) this.x = GAME_WIDTH; if (this.x > GAME_WIDTH) this.x = -this.width; if (this.vy < 0 && keys[this.controls.jump]) { this.vy += JUMP_HOLD_GRAVITY; } else { this.vy += GRAVITY; } this.y += this.vy; this.onGround = false; this.onFrozenPlatform = false; }
    jump() { if (this.onGround && !this.isDead) { this.vy = PLAYER_JUMP; playSound(soundJump); } }
    die(damageAmount) { if (this.isDead) return; this.hp -= damageAmount; playSound(soundLoseLife); if (this.hp <= 0) { this.hp = 0; this.isDead = true; for (let i = 0; i < 50; i++) { particles.push(new Particle(this.x + this.width / 2, this.y + this.height / 2, this.sprite)); } checkGameOver(); } }
    addScore(points) { this.score += points; }
}
class Enemy { constructor(x, y, width, height, sprite) { this.width = width; this.height = height; this.x = x; this.y = y; this.vx = 0; this.vy = 0; this.sprite = sprite; this.isFlipped = false; this.flipTimer = 0; this.onGround = false; this.hitAnimationTimer = 0; this.damage = 10; } draw() { ctx.save(); if (this.hitAnimationTimer > 0 && Math.floor(this.hitAnimationTimer / 50) % 2 === 0) { ctx.restore(); return; } if (this.isFlipped) { ctx.translate(this.x + this.width / 2, this.y + this.height / 2); ctx.rotate(Math.PI); ctx.translate(-(this.x + this.width / 2), -(this.y + this.height / 2)); } ctx.font = '36px sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(this.sprite, this.x + this.width / 2, this.y + this.height / 2); ctx.restore(); } update() { if (this.hitAnimationTimer > 0) this.hitAnimationTimer -= 1000 / 60; if (this.isFlipped) { this.flipTimer -= 1000 / 60; if (this.flipTimer <= 0) { this.isFlipped = false; this.y -= 5; } } if (this.x + this.width < 0 || this.x > GAME_WIDTH) { const spawnPoint = spawnPoints[Math.floor(Math.random() * spawnPoints.length)]; this.x = spawnPoint.x; this.y = spawnPoint.y; this.vy = 0; } this.vy += GRAVITY; this.y += this.vy; this.onGround = false; platforms.forEach(p => { if (this.x < p.x + p.width && this.x + this.width > p.x && this.y + this.height >= p.y && this.y + this.height <= p.y + p.height + 10 && this.vy >= 0) { this.y = p.y - this.height; this.vy = 0; this.onGround = true; if (!p.isFloor) { this.x += p.vx; } } }); } flip() { if (!this.isFlipped) { this.isFlipped = true; this.flipTimer = FLIP_DURATION; this.vy = -5; } } }
...
function startGame(numPlayers, playerData) {
    ...
    if (playerData && playerData.stats) {
        ...
        p1.hp = playerData.stats.hpMax; // Start with full health
...
function setupLevel(levelNum) {
    ...
    for (let i = 0; i < enemyCount; i++) {
        ...
        const x = spawnPoint.x; const y = spawnPoint.y; let enemyType = Math.random();
...
function handleCollisions() {
    players.forEach(p => {
        ...
        if (block.usesLeft > 0 && p.x < block.x + block.width && p.x + p.width > block.x && p.y + p.height >= block.y && p.y + p.height <= block.y + 10 + p.vy && p.vy >= 0) { p.y = block.y - p.height; p.vy = 0; p.onGround = true; }
        let onAnyPlatform = p.onGround;
        let isCurrentlyOnFrozenPlatform = false;
        platforms.forEach(platform => {
            if (p.x < platform.x + platform.width && p.x + p.width > platform.x && p.y + p.height >= platform.y && p.y + p.height <= platform.y + platform.height + p.vy && p.vy >= 0) { p.y = platform.y - p.height; p.vy = 0; p.onGround = true; onAnyPlatform = true; if (platform.isFrozen) isCurrentlyOnFrozenPlatform = true; p.x += platform.vx; }
            if (p.x < platform.x + platform.width && p.x + p.width > platform.x && p.y > platform.y && p.y <= platform.y + platform.height && p.vy < 0) { p.y = platform.y + platform.height; p.vy = 0; const hitCenterX = p.x + p.width / 2; enemies.forEach(enemy => { const onThisPlatform = Math.abs((enemy.y + enemy.height) - platform.y) < 10; const withinHitRange = enemy.x < hitCenterX + 20 && (enemy.x + enemy.width) > hitCenterX - 20; if (!enemy.isFlipped && onThisPlatform && withinHitRange) { enemy.flip(); p.addScore(50); } }); }
        });
        p.onGround = onAnyPlatform;
        p.onFrozenPlatform = isCurrentlyOnFrozenPlatform;
        if (p.x < block.x + block.width && p.x + p.width > block.x && p.y > block.y && p.y <= block.y + block.height && p.vy < 0) { p.y = block.y + block.height; p.vy = 0; block.hit(); }
        enemies.forEach((enemy, index) => { if (p.x < enemy.x + enemy.width && p.x + p.width > enemy.x && p.y < enemy.y + enemy.height && p.y + p.height > enemy.y) { if (enemy.isFlipped) { enemies.splice(index, 1); for (let i = 0; i < 20; i++) particles.push(new Particle(enemy.x, enemy.y, enemy.sprite)); p.addScore(200); } else { p.die(enemy.damage); } } });
    });
...
function drawGameUI() {
    ctx.font = '20px "Press Start 2P", sans-serif'; ctx.textAlign = 'left'; ctx.fillStyle = '#ff4136'; ctx.fillText('P1 SCORE', 40, 30); ctx.fillStyle = 'white'; ctx.fillText((players[0]?.score || 0).toString().padStart(6, '0'), 40, 60);
    if (players[0]) {
        ctx.textAlign = 'left'; ctx.fillStyle = '#ff4136'; ctx.fillText('HP', 40, 92);
        const p1 = players[0]; const barWidth = 180; const hpPercentage = p1.hp / p1.maxHp;
        ctx.fillStyle = '#555'; ctx.fillRect(85, 80, barWidth, 20);
        ctx.fillStyle = '#01FF70'; ctx.fillRect(85, 80, barWidth * hpPercentage, 20);
        ctx.strokeStyle = '#fff'; ctx.strokeRect(85, 80, barWidth, 20);
    }
    ctx.textAlign = 'center'; ctx.fillStyle = '#ff4136'; ctx.fillText('DraicorCoins', GAME_WIDTH / 2, 30); ctx.fillStyle = 'white';
    const totalScore = players.reduce((sum, p) => sum + (p.score || 0), 0);
    const formattedScore = '🪙 ' + totalScore.toLocaleString('en-US'); ctx.fillText(formattedScore, GAME_WIDTH / 2, 60);
    if (players.length > 1 && players[1]) {
        const p2 = players[1]; ctx.textAlign = 'right'; ctx.fillStyle = '#ff4136'; ctx.fillText('HP', GAME_WIDTH - 225, 92);
        ctx.textAlign = 'right'; ctx.fillStyle = '#ff4136'; ctx.fillText('P2 SCORE', GAME_WIDTH - 40, 30); ctx.fillStyle = 'white'; ctx.fillText(p2.score.toString().padStart(6, '0'), GAME_WIDTH - 40, 60);
        const barWidth = 180; const hpPercentage = p2.hp / p2.maxHp;
        ctx.fillStyle = '#555'; ctx.fillRect(GAME_WIDTH - 40 - barWidth, 80, barWidth, 20);
        ctx.fillStyle = '#01FF70'; ctx.fillRect(GAME_WIDTH - 40 - barWidth, 80, barWidth * hpPercentage, 20);
        ctx.strokeStyle = '#fff'; ctx.strokeRect(GAME_WIDTH - 40 - barWidth, 80, barWidth, 20);
    }
}

function drawPlayerSelect() {
    ctx.textAlign = 'center'; ctx.fillStyle = '#ff4136'; ctx.font = '80px "Press Start 2P"'; ctx.fillText('DRAICOR BROS', GAME_WIDTH / 2 + 5, GAME_HEIGHT / 2 - 150 + 5); ctx.fillStyle = '#ffdc00'; ctx.fillText('DRAICOR BROS', GAME_WIDTH / 2, GAME_HEIGHT / 2 - 150);
    if (isMobile) {
        ctx.fillStyle = 'white'; ctx.font = '30px "Press Start 2P"'; ctx.fillText('PRESS START TO PLAY', GAME_WIDTH / 2, GAME_HEIGHT / 2 + 50);
        ctx.fillStyle = 'white'; ctx.font = '16px "Press Start 2P"'; ctx.textAlign = 'left'; ctx.fillText('Player Controls:', 50, GAME_HEIGHT - 230);
        ctx.textAlign = 'left'; ctx.fillText('(◀) Left  (▶) Right  (A) Jump (B) Potion (B+A) Bombs', 50, GAME_HEIGHT - 200);
        ctx.textAlign = 'left'; ctx.fillText('Start/Pause: (Start)', 50, GAME_HEIGHT - 170);
    } else {
        ...
        ctx.font = '16px "Press Start 2P", sans-serif'; ctx.fillStyle = 'white'; ctx.textAlign = 'left'; ctx.fillText('Use Arrow Keys (🠙🠛) to select and Enter (↵) to start', 50, GAME_HEIGHT - 200);
        ctx.textAlign = 'left'; ctx.fillText('Player Controls:', 50, GAME_HEIGHT - 160);
        ctx.textAlign = 'left'; ctx.fillText('P1: (A) Left  (D) Right  (w) Jump (S) Potion (W+S) Bombs', 50, GAME_HEIGHT - 130);
        ctx.textAlign = 'left'; ctx.fillText('P2: (🠘) Left  (🠚) Right  (🠙) Jump (🠛) Potion (🠙🠛) Bombs', 50, GAME_HEIGHT - 110);
        ctx.textAlign = 'left'; ctx.fillText('Start/Pause: (↵) Enter', 50, GAME_HEIGHT - 90);
        ctx.textAlign = 'left'; ctx.fillText('Gamepad supported via JoyToKey (free): joytokey.net/en', 50, GAME_HEIGHT - 50);
    }
}
...
function handleStartPress(playerData) {
    if (gameState === 'playing') { gameState = 'paused'; playSound(soundPause); if (bgMusic) bgMusic.pause(); }
        ...
        startGame(playersToStart, playerData);
        if (bgMusic) { bgMusic.currentTime = 0; bgMusic.play(); musicStarted = true; }
    }
    else if (gameState === 'gameOver') {
        const finalScore = players[0]?.score || 0;
...
export function initializeAndStartGame(playerData, gameOverCallback) {
    ...
    // Setup canvas and initial resize
    ...
    const oldP1JumpHandler = window.p1JumpHandler;
    if(oldP1JumpHandler) window.removeEventListener('keydown', oldP1JumpHandler);
    
    // Keyboard listeners
    window.p1JumpHandler = (e) => {
        const key = e.key.toLowerCase();
        keys[key] = true;
        if (gameState === 'playing') { if (key === 'enter') handleStartPress(playerData); }
        else if (gameState === 'paused' && key === 'enter') { handleStartPress(playerData); }
        else if (gameState === 'playerSelect') { if (!isMobile) { if (key === 'arrowdown') playerSelectOption = 2; if (key === 'arrowup') playerSelectOption = 1; } if (key === 'enter') handleStartPress(playerData); }
        else if (gameState === 'gameOver' && key === 'enter') { handleStartPress(playerData); }
    };

    window.addEventListener('keydown', window.p1JumpHandler);
